const { checkData, getData, deleteData } = require("./database/group_setting")

/**
 * @param {import("@adiwajshing/baileys/src").AnyWASocket} sock
 */
module.exports = joinhandler = async (data, sock) => {
    // For Bot
    const gM = data.action === "add" ? await sock.groupMetadata(data.id) : ''
    const myID = (sock.type === "legacy" ? sock.state.legacy.user.id : (sock.user.id.split(":")[0] + "@s.whatsapp.net" || sock.user.id))
    if (data.action === "add" && data.participants.includes(myID)) {
		const buttons = [
  {buttonId: '!support', buttonText: {displayText: 'Support'}, type: 1},
  {buttonId: '!help', buttonText: {displayText: 'Help'}, type: 1},
]
		/*await sock.sendMessage(data.id, 
		{ 
		    image: { url: "https://telegra.ph/file/21bf4a4de456ca021458c.mp4" }, 
		    caption: "Hello I am Kaguya Sama Made by Das"
            gifPlayback: true			
		}
	)*/
	const buttonMessage = {
    image: {url: 'https://p4.wallpaperbetter.com/wallpaper/468/1009/211/anime-kaguya-sama-love-is-war-kaguya-shinomiya-hd-wallpaper-preview.jpg'},
    caption: "",
    footerText: 'Hello there i am Kaguya.\nNice to meet you\nBot Prefix: !',
    buttons: buttons,
    headerType: 4
} 
await sock.sendMessage(data.id, buttonMessage)
        if (gM.participants.length < 80) {
            await sock.sendMessage(data.id, { text: "Sorry, but this group member is not more than 80 members, I leave soon." })
                .then(async (m) => {
                    await sock.groupLeave(data.id);
                    setTimeout(async () => { await sock.chatModify({ delete: true }, data.id) }, 5000)
                })
        } else {
            await sock.sendMessage(data.id, { text: "Thanks for letting me join your group :D" })
        }
    } else if (data.action === "remove" && data.participants.includes(myID)) {
        let info = checkData(data.id.split('@')[0])
        if (info !== "no_file") { deleteData(data.id.split('@')[0]) }
    }

    // For User
    if (data.action === "add" && !data.participants.includes(myID)) {
        let id = data.id.split('@')[0]
        let info = checkData(id)
        let replace = {
            '%': '%',
            member: data.participants.length > 0 ? (data.participants.map((v) => {
                return "@" + v.split('@')[0]
            })).join(' ') : "@" + data.participants[0].split('@')[0],
            user: data.participants.length > 0 ? (data.participants.map((v) => {
                return "@" + v.split('@')[0]
            })).join(' ') : "@" + data.participants[0].split('@')[0],
            group: gM?.subject,
            desc: gM?.desc?.toString()
        }
        if (info !== "no_file") {
            const dataConf = getData(id)
            let text = dataConf["join"]["msg"].replace(new RegExp(`%(${Object.keys(replace).sort((a, b) => b.length - a.length).join`|`})`, "g"), (_, name) => '' + replace[name])
            if (dataConf["join"]["active"]) { 
                try {
                    ppgroup = await sock.profilePictureUrl(data.id, 'image')
                } catch {
                    ppgroup = 'https://www.linkpicture.com/q/IMG-20220118-WA0387.png'
                }
                await sock.sendMessage(data.id, { image: { url: ppgroup }, caption: `*Group Name* : ${gM?.subject}\n\n*Group Description* : ${gM?.desc?.toString()}` })
            }
        }
    } else if (data.action === "remove" && !data.participants.includes(myID)) {
        let id = data.id.split('@')[0]
        let info = checkData(id)
        let replace = {
            '%': '%',
            member: data.participants.length > 0 ? (data.participants.map((v) => {
                return "@" + v.split('@')[0]
            })).join(' ') : "@" + data.participants[0].split('@')[0],
            user: data.participants.length > 0 ? (data.participants.map((v) => {
                return "@" + v.split('@')[0]
            })).join(' ') : "@" + data.participants[0].split('@')[0],
            group: gM?.subject,
            desc: gM?.desc?.toString()
        }
        if (info !== "no_file") {
            const dataConf = getData(id)
            let text = dataConf["left"]["msg"].replace(new RegExp(`%(${Object.keys(replace).sort((a, b) => b.length - a.length).join`|`})`, "g"), (_, name) => '' + replace[name])
            if (dataConf["left"]["active"]) {
                await sock.sendMessage(data.id, { image: { url: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTFNDoJNNTXaDPKXz8wXvD05Wl3fR0oy65Chw&usqp=CAU" }, caption:"Bye we never gonna miss you"})
            }
        }
    }
}